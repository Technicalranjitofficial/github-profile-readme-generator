...
const AddOns = ({addOns,onCheckAddons}) => {
  return (
    <div className="mt-2 text-slate-300 flex flex-col gap-2">
      <div onClick={()=>onCheckAddons('visitorBadge')} className="w-full border-2 border-slate-800 flex flex-row gap-3 py-3 hover:bg-slate-700 cursor-pointer">
        <input onChange={(e)=>onCheckAddons('visitorBadge')} type="checkbox" checked={addOns.visitorBadge} className="w-20 h-auto accent-green-600 bg-red-400 " name="" id="" />
        <span className="font-Alegreya text-sm">Display Visitors Count Badge</span>
      </div>
      <div onClick={()=>onCheckAddons('twiterBadge')} className="w-full border-2 border-slate-800 flex flex-row gap-3 py-3 hover:bg-slate-700 cursor-pointer">
        <input onChange={(e)=>onCheckAddons('twiterBadge')} type="checkbox" checked={addOns.twiterBadge} className="w-20 h-auto accent-green-600 " name="" id="" />
