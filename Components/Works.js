  ...
  return (
    <div className="w-full mt-5  flex flex-col md:flex-col justify-between">
      ...
      <div className=" w-full flex flex-col mt-2">
        ...
        <input onChange={(e)=>onChange(e,index)} name="workingFor"  value={val.workingFor} className="bg-slate-900 border-2 text-slate-400 text-sm font-OpenSans outline-none border-slate-800 py-2  px-2" type="text" />
        
      </div>{" "}
      <div className=" w-full flex flex-col mt-2">
        <label htmlFor="Workname" className="mb-1  text-slate-400 text-md font-Montserrat">Project Name</label>
        <input onChange={(e)=>onChange(e,index)} value={val.pName} name="pName" className="bg-slate-900 border-2 text-slate-400 text-sm font-OpenSans outline-none border-slate-800 py-2  px-2" type="text" />
      </div>{" "}
      <div className=" w-full flex flex-col mt-2">
        <label htmlFor="Workname" className="mb-1  text-slate-400 text-md font-Montserrat">Project Link</label>
        <input onChange={(e)=>onChange(e,index)} value={val.pLink} name="pLink" className="bg-slate-900 border-2 text-slate-400 text-sm font-OpenSans outline-none border-slate-800 py-2  px-2" type="text" />
      </div>
      </div>
