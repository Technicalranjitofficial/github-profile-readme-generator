...
const Message = ({msg,success,setMessage}) => {
  ...
  return (
    <div className={`fixed md:w-screen h-20 ${msg.length>0?"top-3":"-top-96"} duration-300 ease-linear z-50`}>
        ...
        <span className='text-slate-200 font-Alegreya font-semibold'>{msg}</span>
        </div>
    </div>
  )
}
